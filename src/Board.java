...
public class Board {
    ...
    public boolean movePlayer(Player player, char direction) {
        ...
        if (playerCell == null) {
            return false;
        } //if the player isn't on the board, break from the method
        ...
        switch (direction) {
            case 'u': //move up
                if (playerCell.row == 0) {
                    return false;
                } //prevent player trying to move out of bounds
                target = cells.get(playerCell.row - 1).get(playerCell.col);
                break;
            case 'd': //move down
                if (playerCell.row == 24) {
                    return false;
                } //prevent player trying to move out of bounds
                target = cells.get(playerCell.row + 1).get(playerCell.col);
                break;
        ...
        if (!target.type.equals("   ")) {
            return false;
        } //if target cell is not empty, the move is invalid
        if (target.player != null) {
            return false;
        } //can't move into a square occupied by another player
    ...
    public void kidnapPlayer(Player victim, Room destination) {
        ...
        if (locat != null && locat.equals(destination)) {
            return;
        } //player not found or is already in the room, do nothing
    ...
    public void moveWeapon(String weapon, Room start) {
        if (start.weapon != null && start.weapon.equals(weapon)) {
            return;
        } //if the weapon is already here, do nothing
